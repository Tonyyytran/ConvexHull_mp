...
{
    deque<Point> s1, s2;       // sub-queues
    divide(q, s1, s2);         // divide in two sub-queues.
    // if 1 or less processes no need to fork
    if(leaves <= 1)
    {
        // divide and conquer if dequeue is larger than 1
        if (s1.size() > 1 )
        {
        ...
        // create pipe and fork. child sends data to parent through pipe, divide and conquer if dequeue is larger than 1
    else
    {
        // create pipe
        int pipeFD[2];
        ...
        if(pid == 0)   // child process
        {
            ...
            {
                //close unused read side pipe
                close(pipeFD[0]);
                //send data through pipe
                send(pipeFD[1], s2);
                //close write side pipe when done writing
                close(pipeFD[1]);
            }
        ...
        else   // parent process
        {
            if (s1.size() > 1 )
            {
                divide_and_conquer(s1, leaves - 1);  // work on the left queue
            }
            int status;
            wait(&status);
            //close unused write side pipe
            close(pipeFD[1]);
            //here parent will read what child has written
            recv(pipeFD[0],s2);
            //close read side pipe when done
            close(pipeFD[0]);

            merge(q, s1, s2);          // merge right and left side
            graham(q);                 // apply the graham algorithm
        }
    }
}
